...
if ($^O eq 'MSWin32') {
  ...
  $platex = 'platex %O -synctex=1 -interaction=nonstompode %S';
  $pdflatex = 'pdflatex %O -synctex=1 -interaction=nonstopmode %S';
  $lualatex = 'lualatex %O -synctex=1 -interaction=nonstopmode %S';
  $lualatexBsn = "find . -type f -name '*.tex' | xargs sed -i '' -e 's/、/，/g' -e 's/。/．/g'; lualatex %O -synctex=1 -interaction=nonstopmode %S";
